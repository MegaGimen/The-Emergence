...
class SpiderCrawl:
    ...
    async def _find(self, rpcmethod,datas):
        """
        ...
        for peer in self.nearest.get_uncontacted()[:count]:
            ...
            dicts[peer.id] = rpcmethod(peer, self.node,datas)
            self.nearest.mark_contacted(peer)
        found = await gather_dict(dicts)
        return await self._nodes_found(found,datas)
...
class NodeSpiderCrawl(SpiderCrawl):
    async def find(self,datas):
        """
        Find the closest nodes.
        """
        return await self._find(self.protocol.call_find_node,datas)

    async def _nodes_found(self, responses, datas):
        """
        Handle the result of an iteration in _find.
        """
        toremove = []
