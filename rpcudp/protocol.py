...
class RPCProtocol(asyncio.DatagramProtocol):
    ...
    """
    def __init__(self, wait_timeout=6):
        """
    ...
    def datagram_received(self, data, addr):
        LOG.warning("received datagram from %s", addr)
    ...
    def _accept_response(self, msg_id, data, address):
        print(f'_accrpt_response msgid={msg_id}')
    ...
    async def _accept_request(self, msg_id, data, address):
        print(f'_accpt_request msgid={msg_id}')
        #显式另存一下msg_id
    ...
    def __getattr__(self, name):
        ...
        def func(address, *args):
            ...
            #return None
            print(f'debug,name={name},args={args},address={address}')
            self.transport.sendto(txdata, address)
