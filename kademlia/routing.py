...
class KBucket:
    ...
    def add_node(self, node):
        ...
        if node.id in self.nodes:
            ...
            print(f'桶里的节点,False:{self.nodes.values()}')
            return False
        
        print(f'桶里的节点,True：{self.nodes.values()}')
        return True
...
class RoutingTable:
    ...
    def print_bucket(self):
        for _ in range(len(self.buckets)):
            print(f'桶里的节点{_}：{self.buckets[_].nodes.values()}')
    ...
    def add_contact(self, node):
        print(f'add contact is called:{node}')
        index = self.get_bucket_for(node)
    ...
    def find_neighbors(self, node, k=None, exclude=None):
        ...
        print(f'在小根堆中，{nodes}最后要被发出了，而这个是在find_neightbors中被调用的')
